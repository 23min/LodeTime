...
After evaluating GitHub's Spec Kit against our custom AI framework in `ai/`, we've determined that **our framework is better suited for this project**. While Spec Kit excels at feature-level specification-driven development, our framework provides the epic-level orchestration, human oversight, and brownfield flexibility required for Treehouse.
...
**[Kiro](https://kiro.dev/docs/specs/):** Spec-driven development platform with structured specification format, product specs as source of truth
...
## Why NOT Use Spec Kit for Treehouse
...
- Migration cost to restructure existing `dev/` hierarchy
...
**Treehouse needs epic orchestration:**
```
Epic: Critical Flow Reporting (6+ months)
├── M-critical-flow-01: Data Pipeline Setup (3 weeks)
├── M-critical-flow-02: Kusto Query Migration (2 weeks)
├── M-critical-flow-03: API Layer (3 weeks)
├── M-critical-flow-04: UI Dashboard (4 weeks)
└── Epic Wrap → Release v1.0.0 → Git tag

Epic: Infrastructure Template (4+ months)
├── M-infra-01: Bicep Template Foundation
├── M-infra-02: AI-Assisted Deployment
...
```
...
- Would need to migrate all `dev/epics/` and `dev/milestones/` to Spec Kit structure
...
**Create:** `dev/development/principles.md`
...
**No immediate changes required.** Our framework is production-ready (v1.0.0) and well-suited for Treehouse's needs.
...
**Overall:** Our framework wins on most criteria that matter for Treehouse. Spec Kit excels at feature-level work, but we need epic-level orchestration.
...
**For Treehouse:** Our framework is the right tool for the job.
