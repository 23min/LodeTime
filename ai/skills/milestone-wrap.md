...
In the milestone spec file (`dev/milestones/<milestone-id>.md`), update the header:
...
3. **Milestone Release Notes** (required)
   - Add a "Release Notes" section to the milestone spec document itself (`dev/milestones/<milestone-id>.md`)
...
- Milestone specs remain in `dev/milestones/` (root) until the entire epic completes
- Tracking docs stay in `dev/milestones/tracking/`
- Only archive specs to `dev/milestones/completed/` when epic wraps (via epic-wrap skill)
